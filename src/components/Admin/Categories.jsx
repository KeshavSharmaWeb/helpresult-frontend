...
const Categories = () => {
    ...
    const handleCancel = (id) => {
        axios.post(url + "/delete-category", {id: id}).then(res => {
            if(res.status === 200) {
    ...
    const handleUpdate = (id, name) => {
        axios.post(url + "/update-category", {id: id, name: name}).then(res => {
            if(res.status === 200) {
            ...
            <Table striped bordered hover style={{ marginTop: 30, textAlign: "center" }}>
                ...
                <tbody>
                    ...
                    categories.map((category, index) => (
                        ...
                        <td><Button variant="outline-dark" size="sm" onClick={()=> {handleUpdate(category._id, document.getElementById(`name-${category._id}`).value);document.getElementById(`name-${category._id}`).value=""}}>Update</Button></td>
                    </tr>
                    )
                    )
                    }
